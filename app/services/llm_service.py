...
def _build_ollama_payload(messages: list[ChatMessage],*,stream: bool) -> dict[str,object]:
    return {
...
async def _stream_chat_with_client(messages: list[ChatMessage],*,client: httpx.AsyncClient) -> AsyncIterator[StreamEvent]:
    url = f"{settings.ollama_base_url.rstrip('/')}/api/chat"
...
async def stream_chat_with_llm(messages: list[ChatMessage],*,client: httpx.AsyncClient | None = None) -> AsyncIterator[StreamEvent]:
    if client is not None:
